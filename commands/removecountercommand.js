module.exports = {
	name: 'removecountercommand',
	description: 'removes a custom counter command from your channel',
	async execute(channel, tags, message, client, sql, authProvider, followerchannels, TEclient) {
		...
		let result = await sql`SELECT * FROM countercommands WHERE username=${String(channelName)} AND command=${String(command)};`;
		...
		await sql`DELETE FROM countercommands WHERE username=${String(channelName)} AND command=${String(command)};`;
